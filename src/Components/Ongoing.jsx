...
function Ongoing({ rendered, prevPage }) {

    const { ongoingAnime, isSearch, searchResults, getOngoingAnime, handleSubmit,
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            (!isSearch) ? getOngoingAnime(controller.signal, page) : handleSubmit(controller.signal);
        }, 350);
        ...
        <div>
            <div className="popular-stripe">
                ...
                <div className="next-prev-pages">
                    ...
                    }} disabled={(!isSearch) ? (!ongoingAnime?.length) : (!searchResults?.length)} >{(!isSearch) ?
